/**
 * Cyberpunk Sci-Fi Telemetry Controller & Dynamic Lab Sensors Engine
 * Includes Interactive DAY / WEEK / LIVE Filter for Analytics Dashboard
 */

let sciFiChart = null;
let pollTimer = null;
let rawRecordsCache = [];
let currentViewMode = 'LIVE'; // Options: 'LIVE', 'DAY', 'WEEK'
...
function processTelemetryRecords(records) {
  ...
  // 1. Center Radial Arc Gauge (DHT11 Temperature 0 - 60 °C)
  ...
  // 2. Left Cards (Node Health & MQ-2 Gas)
  ...
  if (latest.gasDetected || gasRaw > 350) {
    ...
    gasStatusBox.innerText = 'SAFE • NO GAS LEAK';
    gasStatusBox.className = 'status-box box-safe';
  }

  // 3. Security & Motion Tracking Table (Sparklines)
  ...
  if (motion) {
    ...
    pirValText.innerText = 'NO MOTION';
  }
  ...
  // 4. Lab Environment Threshold Bars (Bottom Right)
  const tempRatio = Math.min(100, (temp / 50) * 100);
  document.getElementById('barTempText').innerText = `${tempRatio.toFixed(1)}% (${Number(temp).toFixed(1)}°C)`;
  ...
  document.getElementById('barDistText').innerText = `${distRatio.toFixed(1)}% (${Number(distance).toFixed(1)}cm)`;
  document.getElementById('barDistFill').style.width = `${distRatio}%`;

  // 5. Update Sci-Fi Analytics Chart based on selected Time Mode (LIVE, DAY, WEEK)
  updateAnalyticsChartMode(records);

  // 6. Render Raw Table
  renderTable(records);
}

// Filter and Update Chart based on Mode (LIVE, DAY, WEEK)
function updateAnalyticsChartMode(records) {
  let chartRecords = [];
  let formatLabelFn = (r) => new Date(r.receivedAt || Date.now()).toLocaleTimeString();

  const now = Date.now();

  if (currentViewMode === 'LIVE') {
    // Show recent 12 real-time packets
    chartRecords = [...records].reverse().slice(-12);
  } else if (currentViewMode === 'DAY') {
    // Show packets from last 24 Hours
    const dayMs = 24 * 60 * 60 * 1000;
    const dayFiltered = records.filter(r => (now - new Date(r.receivedAt || now).getTime()) <= dayMs);
    chartRecords = dayFiltered.length > 0 ? [...dayFiltered].reverse() : [...records].reverse();
    formatLabelFn = (r) => {
      const d = new Date(r.receivedAt || Date.now());
      return `${d.getHours()}:00`;
    };
  } else if (currentViewMode === 'WEEK') {
    // Show packets from last 7 Days
    const weekMs = 7 * 24 * 60 * 60 * 1000;
    const weekFiltered = records.filter(r => (now - new Date(r.receivedAt || now).getTime()) <= weekMs);
    chartRecords = weekFiltered.length > 0 ? [...weekFiltered].reverse() : [...records].reverse();
    formatLabelFn = (r) => {
      const d = new Date(r.receivedAt || Date.now());
  ...
  const timestamps = chartRecords.map(formatLabelFn);
  ...
  const chartBadge = document.getElementById('chartPacketBadge');
  if (chartBadge) chartBadge.innerText = `${chartRecords.length} ${currentViewMode === 'LIVE' ? 'PACKETS' : 'RECORDS'}`;
}
...
// Setup Event Listeners, Button Click Handlers & Auto-Polling
document.addEventListener('DOMContentLoaded', () => {
  initSciFiChart();
  fetchTelemetry();

  // Setup Time Pill Buttons Listener (DAY, WEEK, LIVE)
  const timePillBtns = document.querySelectorAll('.time-pill');
  timePillBtns.forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      updateAnalyticsChartMode(rawRecordsCache);
    });
  });
